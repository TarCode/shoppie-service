...
export class ItemController {
  ...
  public setRoutes() {
    ...
    this.router.route("/list/:id")
      .get(this.findByListId);
  ...
  private sayHello = (_: Request, res: Response) => {
    const welcomeMessage = this.itemService.welcomeMessage();
  ...
  private delete = async (req: Request, res: Response) => {
    try {
      const deleteItemResult = await this.itemService.delete(
        req.params.id
      );
