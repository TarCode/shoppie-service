...
export class ItemService {
  ...
  public async delete(id: string) {
    const deletedItem: (IItem & { _id: ObjectId }) | null = await Item.findByIdAndDelete(id).exec()
  ...
  public async update(id: string, item: IItem) {
    const updatedItem: (IItem & { _id: ObjectId }) | null = await Item.findByIdAndUpdate(id, item).exec()
